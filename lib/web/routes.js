...
exports.install = function(app, devops){
	...
	var v1_request = function(project, pull_request_title){
		...
		identifier_match = title.match(/([B|D]\d{4,}){1}/);
		if (!identifier_match){
		...
		type = split[0];
		if (type !== "B" && type !== "D"){
			throw Error('unknown asset type: ' + asset_type);
		}
		...
		return _.bind(utils.request_maker, null, options);
	};
	...
	app.post(urls.PUSH_API, [express.bodyParser()], function(req, res){
		...
		var story_points = settings.DEFAULT_STORY_POINTS;
		...
		console.log(action, e.title, e.pull_request.user.login, e.sender.login, e.pull_request.merged, e.pull_request.merged_by);
		...
		async.parallel(requests, function(err, results){
			...
			if (results.version_one){
				...
				if (asset && asset[0]._children[0].attrib.name === "Estimate")
					story_points = asset[0]._children[0].text || settings.DEFAULT_STORY_POINTS;
				}
