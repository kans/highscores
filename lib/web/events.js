...
Event = utils.make_class({
	...
	points: function(){
		var self = this;
		if (self.status && self.story_points){
			return self.multiplier * self.story_points;
		}
		return 1;
	}
});
...
exports.Git_Event = function(user, status, story_points, date){
	return new Event(null, user, status, story_points, date);
};

exports.V1_Event = function(user, status, story_points, date){
	return new Event(user, null, status, story_points, date);
};
