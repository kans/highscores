...
var delete_hooks = function(results, callback){
	...
	_.each(results, function(result, repo){
		if (result.res.statusCode === 401){
			return callback(new Error('Your config is probably wrong for github- they don\'t like your creds.'));
		...
		_.each(JSON.parse(result.data), function(hook){
			...
			if (hook.config.url === get_api_url(repo)){
				if (_.indexOf(to_keep, hook.config.url) === -1 && hook.active === true &&
					hook.name === 'web' && _.indexOf(hook.events, 'pull_request') !== -1){
					console.log("KEEPing hook: " + hook.url, " for url", hook.config.url);
