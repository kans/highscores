...
var get_hooks = function(){
	...
	async.parallel(list_hooks, console.log);
};
...
var parse_hooks_list = function(results, callback){
	...
	_.each(results, function(result, repo){
		var hooks_list = JSON.parse(result.data);
		var hook_exists = false;
		var i, hook;
		...
		for (i=0; i<hooks_list.length; i++){
			hook = hooks_list[i];
			hook_exists = (hook.name === "web" &&
				hook.config.url === util.format("http://%s:%s/api/github/%s",
					settings.external_ipv4, settings.listen_port, repo) &&
				_.indexOf(hook.events, "pull_request") !== -1 &&
				hook.active === true
			);
			// if we find a hook, don't make another
			if (hook_exists){
				console.log(hook);
				break;
			}
		}
		// if we don't find a hook, add one
		if (!hook_exists){
...
var add_hooks = function(hooks_to_create, callback){
	...
	_.each(hooks_to_create, function(repo){
		...
		options.post_data = {
			...
			events: ["pull_request"],
			config: {
				url: util.format("http://%s:%s/api/github/%s", settings.external_ipv4, settings.listen_port, repo),
				content_type: "json"
			}
		};
