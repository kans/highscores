...
var github_request = utils.make_class({
	init: function(repo, devops, method, id){
		var self = this;
		self.method = method;
		// TODO: safify
		self.path = util.format("/repos/%s/%s/hooks", devops.org, repo);
		if (id){
			self.path += '/' + id;
		}
		self.headers = {
			'Authorization': utils.create_basic_auth(devops.username, devops.apikey)
		};
	},
	host: "api.github.com"
});
...
exports.install = utils.make_class({
	init: function(devops){
		...
		self.devops = {};
		// store all well defined devops internally
		_.each(devops, function(devops, repo){
			github_api = null;
			try{
				github_api = devops.related_apis.github;
			}catch(e){}
			if (github_api && github_api.org && github_api.username && github_api.apikey){
				self.devops[repo] = github_api;
			} else{
				console.log('ignoring devops: ', repo);
			}
		});
...
var	get_hooks = function(callback){
	...
	var list_hooks = {};
	var options;
	_.each(self.devops, function(github_api, repo){
		options = new github_request(repo, github_api, "GET");
			...
			_.each(JSON.parse(result.data), function(hook){
				...
				if (hook.config.url === get_api_url(repo)){
					...
					if (!delete_hook){
						...
						options = new github_request(repo, self.devops[repo], "DELETE", hook.id);
...
var add_hooks = function(results, existing_hooks, callback){
	...
	for (repo in self.devops){
		...
		options = github_request(repo, self.devops[repo], "POST");
